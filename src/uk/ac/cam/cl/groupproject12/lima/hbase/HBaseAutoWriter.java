 ...
 * <p/>
 * 1) the unqualified class name of the AutoWritable is the same as the name of a table in HBase<br /> 2) for every
 * field in the AutoWritable descendant type there is a corresponding HBase column with name "f1:[col name]"
 *
 * @see AutoWritable
 */
public abstract class HBaseAutoWriter {
     ...
     *
     * @param type
     *         A class that extends AutoWritable.
     *
    ...
    /**
     * Grabs the byte array key for any AutoWritable. Generates bytes from fields annotated with HBaseKey, representing
     * them in their declared order as strings joined by a delimiter character.7
     *
     * @param w
     *         AutoWritable to analyse.
     *
    ...
    /**
     * Inserts/updates a row in the HBase table corresponding to the given AutoWritable. Each field is put in its own
     * column.
     * <p/>
     * Requires that all the fields are non-null writables.
     *
     * @param w
     *         AutoWritable containing the data to update from.
     */
     ...
     *
     * @param type
     *         Input on which getTableName() can be called.
     * @param key
     *         The byte[] value of the HBase key.
     *
     ...
     *
     * @param w
     *         AutoWritable on which getTableName() is to be called.
     *
     ...
     * @param key
     *         Key of the row to get.
     * @param w
     *         AutoWritable to collect the result set in.
     *
     * @throws IOException
     */
