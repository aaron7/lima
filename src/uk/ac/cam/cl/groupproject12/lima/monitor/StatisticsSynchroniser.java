...
/**
 * Statistics synchroniser synchronises the stats on a per-router basis from HBase to PostgreSQL, aggregating them along
 * the way. Due to the manner in which the EventMonitor invokes the synchronisers, this is package visible only.
 */
class StatisticsSynchroniser implements IDataSynchroniser {
    ...
    /**
     * Reads data out of HBase, processes it to calculate the relevant moving averages over greater periods of time than
     * that of the logs, then writes the result into PostgreSQL.
     *
     * @param monitor
