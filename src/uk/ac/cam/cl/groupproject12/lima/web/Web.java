...
public class Web {
     ...
     * @param ip IP of the router status to update
     * @param status 0 if router updated, 1 to increment
     */
    public static void newJob(String ip, String timestamp, int numOfJobs){
        Web.httpRequest("newJob?ip="+ip+"&timestamp="+timestamp+"&numOfJobs="+numOfJobs);
    }
    
    /**
     * send updates of the status of router jobs to the web UI
     * @param ip IP of the router status to update
     * @param allComplete true is the entire set of jobs is complete otherwise we inc
     */
    public static void updateJob(String ip, String timestamp, boolean allComplete){
        Web.httpRequest("updateJob?ip="+ip+"&timestamp="+timestamp+"&inc="+(allComplete ? 1: 0));
    }
